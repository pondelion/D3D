...
private:
	ID3DXMesh *mVectorMesh, *mTestMesh;
	std::vector<MTRL> mVectorMtrl, mTestMtrl;
	std::vector<IDirect3DTexture9*> mVectorTex, mTestTex;
...
test1d3d::~test1d3d()
{
	...
	for(int i=0; i < mVectorTex.size(); ++i)
	{
	if(mVectorTex[i])
	{
	mVectorTex[i]->Release();
	mVectorTex[i] = NULL;
	}
	}
...
void test1d3d::initDirect3D()
{
	...
	createMeshGridBuffer(100, 100, 0.5, 0.5, D3DXVECTOR3(0.0, 0.0, 0.0));
	createMeshGridFX();
	...
	createLightTexGridBuffer(100, 100, 0.5, 0.5, D3DXVECTOR3(0.0, 0.0, 0.0), "..\\resource\\img\\ground0.dds");
	...
	mText.push_back(MYTEXT3D("aiueokakikukeko\nあいうえおかき", D3DXVECTOR3(0.0f, 3.0f, 0.0f), D3DXCOLOR(0xff00ffff), 80, 10,md3dDevice));
