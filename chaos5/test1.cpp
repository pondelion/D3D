...
void test1d3d::initDirect3D()
{
	...
	createLightTexGridBuffer(101, 101, 0.1, 0.1, D3DXVECTOR3(0.0, 0.0, 0.0), "..\\resource\\img\\wall21_c.jpg");
...
void test1d3d::update(float dt)
{
	...
	mCamPos.x = 10.0f * cos(0.2f*mtime);
	mCamPos.z = 8.0f * sin(0.2f*mtime);
	/*
	vy += -10.0f*dt;
	y += vy*dt;
	if(y <= 0.5f)
...
float calcHeight(float x, float z, int n, int m)
{
	float y=0.0f,tempy=0.0f;
	int i,j;
	DWORD t;
	t = GetTickCount();
	for(i=1;i<=n;i++)
	{
		tempy = sin((float)i*D3DX_PI*0.5f)*cos((float)i*D3DX_PI*0.1f*x)/(float)i;
		for(j=1;j<=m;j++)
		{
		y += tempy*sin((float)j*D3DX_PI*0.5f)*cos((float)i*D3DX_PI*0.1f*z)/(float)j;
		
		}
	}
	y *= 4.0f/(D3DX_PI*D3DX_PI);
	//y += 0.25f;
	//fout << "x=" << x << ", y=" << z << std::endl;
	return y;
}
