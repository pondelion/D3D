...
LRESULT CALLBACK MyD3DWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	case WM_CHAR:
		...
		if(myd3d)
		{
			if(wp == 119 || wp == 97 || wp == 115 || wp == 100 || wp == 120)
			{
...
d3dclass::d3dclass(HINSTANCE hInst, HWND hwndParent, LPSTR lpstrCaption, D3DDEVTYPE devtype, DWORD VP,RECT *rc)
{
	...
	mLightPos = D3DXVECTOR3(5.0f, 20.0f, 2.0f);
	mLightVec = -mLightPos;
	...
	mLTGridMtrl = MTRL(D3DXCOLOR(0.3f, 0.3f,0.3f, 0.6f),D3DXCOLOR(0.6f, 0.6f,0.6f, 0.6f), D3DXCOLOR(0.7f, 0.7f,0.7f, 0.4f), 16.0f);
...
void d3dclass::initDirect3D()
{
	...
	if(FAILED(D3DXCreateTextureFromFile(md3dDevice, "../resource/img/red.GIF", &mRedTex)))
	{
		MessageBox(NULL, "D3DXCreateTextureFromFile(md3dDevice, ""../resource/img/red.GIF"", &mRedTex) failed", "", MB_OK);
		return;
	}
...
void d3dclass::d3drelease()
{
	...
	for(int i=0; i < mArrowTex.size(); ++i)
	{
	if(mArrowTex[i])
	{
	mArrowTex[i]->Release();
...
void d3dclass::createMeshGridFX()
{
	...
	{
	ID3DXBuffer* error = 0;
	if(FAILED(D3DXCreateEffectFromFile(md3dDevice, "gridfx.fx", 0, 0,
...
void d3dclass::createPointLightFX()
{
	...
	{
	ID3DXBuffer* error = 0;
	if(FAILED(D3DXCreateEffectFromFile(md3dDevice, "pointlightfx.fx", 0, 0,
...
void d3dclass::createLightTexFX()
{
	...
	ID3DXBuffer* error = 0;
	if(FAILED(D3DXCreateEffectFromFile(md3dDevice, "lighttexfx.fx",
...
void d3dclass::drawSphere()
{
	...
	D3DXMatrixScaling(&S, mSphereScale.x, mSphereScale.y, mSphereScale.z);
...
void d3dclass::createLightTexGridBuffer(int nVR, int nVC, float dx, float dz, const D3DXVECTOR3& center, LPCSTR texname)
{
	...
	for(int i = 0; i < nVR; ++i)
	{
		...
		vpnt[k].tex = D3DXVECTOR2((float)j, (float)i)*0.05f;
		}
	}
...
void d3dclass::createArrow(LPCSTR filename)
{
	LoadXfile(filename, mArrowMtrl, mArrowTex, &mArrowMesh);
	mText.push_back(MYTEXT3D("X", D3DXVECTOR3(6.0f, 0.5f, 0.0f), D3DXCOLOR(0xffff0000), 30, 1,md3dDevice));
	mText.push_back(MYTEXT3D("Z", D3DXVECTOR3(0.0f, 6.5f, 0.0f), D3DXCOLOR(0xff00ff00), 30, 1,md3dDevice));
	mText.push_back(MYTEXT3D("Y", D3DXVECTOR3(0.0f, 0.5f, 6.0f), D3DXCOLOR(0xff0000ff), 30, 1,md3dDevice));
}

void d3dclass::drawLTXYZaxis(D3DXVECTOR3& org,float size)
{
	if(FAILED(mLightTexFX->SetTechnique(mhLT.mhTech)))
		MessageBox(NULL, "SetTechnique() failed", "", MB_OK);
	...
	if(FAILED(mLightTexFX->SetFloat(mhLT.mhPower, mLTBoxMtrl.specPower)))
		MessageBox(NULL, "SetFloat() failed", "", MB_OK);
	if(FAILED(mLightTexFX->SetTexture(mhLT.mhTex, mGreenTex)))
		MessageBox(NULL, "SetTexture() failed", "", MB_OK);
	
	
	D3DXMATRIX	W,WIT,S,R;
	D3DXMatrixScaling(&S, size*0.2f,  size, size*0.2f);
	D3DXMatrixTranslation(&W, org.x, org.y, org.z);
	D3DXMatrixInverse(&WIT, 0, &(S*W));
	D3DXMatrixTranspose(&WIT, &WIT);
	...
	for(int j = 0; j < mArrowMtrl.size(); ++j)
	{
		...
		if(mArrowTex[j] != 0)
		{
			mLightTexFX->SetTexture(mhLT.mhTex, mArrowTex[j]);
		}
		...
		if(!mArrowMesh)
			testes;
			*/
		mArrowMesh->DrawSubset(j); // z-axis
		
	}
	
	D3DXMatrixRotationZ( &R, -D3DX_PI*0.5f );
	
	if(FAILED(mLightTexFX->SetMatrix(mhLT.mhW, &(S*R*W))))
		MessageBox(NULL, "SetMatrix() failed", "", MB_OK);
	if(FAILED(mLightTexFX->SetMatrix(mhLT.mhWIT, &WIT)))
		MessageBox(NULL, "SetMatrix() failed", "", MB_OK);
	if(FAILED(mLightTexFX->SetMatrix(mhLT.mhWVP, &(S*R*W*mView*mProj))))
		MessageBox(NULL, "SetMatrix() failed", "", MB_OK);
	if(FAILED(mLightTexFX->SetTexture(mhLT.mhTex, mRedTex)))
		MessageBox(NULL, "SetTexture() failed", "", MB_OK);
	...
	for(int j = 0; j < mArrowMtrl.size(); ++j)
	{
		...
		if(!mArrowMesh)
		testes;
		mArrowMesh->DrawSubset(j); // x-axis
	...
	if(FAILED(mLightTexFX->SetMatrix(mhLT.mhWVP, &(W*mView*mProj))))
		MessageBox(NULL, "SetMatrix() failed", "", MB_OK);
	if(FAILED(mLightTexFX->SetTexture(mhLT.mhTex, mBlueTex)))
		MessageBox(NULL, "SetTexture() failed", "", MB_OK);
	for(int j = 0; j < mArrowMtrl.size(); ++j)
	{
		...
		if(!mArrowMesh)
		testes;
		mArrowMesh->DrawSubset(j); // y-axis
...
void d3dclass::drawLTArrow(D3DXVECTOR3& pos,float size,D3DXMATRIX& R,IDirect3DTexture9* tex)
{
	if(FAILED(mLightTexFX->SetTechnique(mhLT.mhTech)))
		MessageBox(NULL, "SetTechnique() failed", "", MB_OK);
	...
	if(FAILED(mLightTexFX->SetFloat(mhLT.mhPower, mLTBoxMtrl.specPower)))
		MessageBox(NULL, "SetFloat() failed", "", MB_OK);
	if(FAILED(mLightTexFX->SetTexture(mhLT.mhTex, tex)))
		MessageBox(NULL, "SetTexture() failed", "", MB_OK);
		
		D3DXMATRIX	W,WIT,S;
	D3DXMatrixScaling(&S, size*0.8f,  size, size*0.8f);
	D3DXMatrixTranslation(&W, pos.x, pos.y, pos.z);
	D3DXMatrixInverse(&WIT, 0, &(S*R*W));
	D3DXMatrixTranspose(&WIT, &WIT);
