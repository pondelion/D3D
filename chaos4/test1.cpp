...
float calcHeight(int i, int j,float dt,float time,int yind);
void onChange(IDirect3DVertexBuffer9* buf,std::vector<MYTEXT3D> mText);
float initialPos(int i,int j);
float initialVel(int i,int j);
float fPerMass(int i,int j, float t);

extern int N,M;
D3DXVECTOR3 lightPos=D3DXVECTOR3(0.0f,0.0f,0.0f);
extern bool bchanged;
float height1[3][MESHN][MESHM];
int y_index;
...
: D3DParticleClass(d3dc,fxname,techname, texname, box,maxparticle,emitionrate)
	...
	p->initColor = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
	randomVec(p->initVel);
	p->initVel += D3DXVECTOR3(0.0f, 2.0f, 0.0f);
	
	//float theta = randomFloat(0, 2.0f*D3DX_PI);
	//float r = randomFloat(5.0f, 7.0f);
	D3DXVECTOR3 r;
	randomVec(r);
	p->initPos.x = lightPos.x;
	p->initPos.y = lightPos.y;
	p->initPos.z = lightPos.z;
	p->initPos += 0.6*r;
	mDParticle.pop_back();
...
: d3dclass(hInst, hwndParent, lpstrCaption, devtype, VP,rc)
{
	...
	/*
	LineVec.push_back(VPosCol(D3DXVECTOR3(5.0f, 0.0f, 5.0f), 0xffff00ff));
	LineVec.push_back(VPosCol(D3DXVECTOR3(-5.0f, 0.0f, 5.0f), 0xffff00ff));
	LineVec.push_back(VPosCol(D3DXVECTOR3(-5.0f, 0.0f, -5.0f), 0xffff00ff));
	LineVec.push_back(VPosCol(D3DXVECTOR3(5.0f, 0.0f, -5.0f), 0xffff00ff));
	LineVec.push_back(VPosCol(D3DXVECTOR3(5.0f, 0.0f, 5.0f), 0xffff00ff));
...
void test1d3d::initDirect3D()
{
	...
	if(FAILED(D3DXCreateTextureFromFile(md3dDevice, "..\\resource\\img\\blue.jpg", &mBlueTex)))
	{
		MessageBox(NULL, "D3DXCreateTextureFromFile(md3dDevice, ""../resource/img/blue.jpg"", &mRedTex) failed", "", MB_OK);
		return;
	}
	if(FAILED(D3DXCreateTextureFromFile(md3dDevice, "..\\resource\\img\\green.jpeg", &mGreenTex)))
	{
		MessageBox(NULL, "D3DXCreateTextureFromFile(md3dDevice, ""../resource/img/green.jpeg"", &mRedTex) failed", "", MB_OK);
		return;
	}
	if(FAILED(D3DXCreateTextureFromFile(md3dDevice, "..\\resource\\img\\red.jpg", &mRedTex)))
	{
		MessageBox(NULL, "D3DXCreateTextureFromFile(md3dDevice, ""../resource/img/red.jpg"", &mRedTex) failed", "", MB_OK);
		return;
	}
	...
	createSphere("..\\resource\\img\\sun1.jpg", 1.0f, D3DXVECTOR3(0.0f, 4.0f, 0.0f));

	createLightTexGridBuffer(MESHN, MESHM, 0.25f, 0.25f, D3DXVECTOR3(0.0, 0.0, 0.0), "..\\resource\\img\\water5.jpg");
	...
	*/
	...
	for(int i = 0; i < MESHN; ++i)
	{
		...
		vpnt[k].pos.y = initialPos(i,j);
		height1[y_index][i][j] = vpnt[k].pos.y;
		
		
		if(i != 0 && j != 0)
		{
		D3DXVec3Cross(&n,&(vpnt[k-MESHN].pos-vpnt[k].pos) , &(vpnt[k-1].pos-vpnt[k].pos));
		D3DXVec3Normalize(&n, &n);
		//if(k==5100 || k==5101 || k==5201)
		//fout << "n.y=" << n.y << std::endl;
	...
	for(int i = 0; i < MESHN; ++i)
	{
		...
		height1[y_index][i][j] = height1[y_index-1][i][j]+0.1f*initialVel(i,j);
	...
	*/
	/*
	N = M = 16;
	onChange(mVBLTGrid,mText);
	if(N>9)
	{
	mText[3].text = "n=m=";
	mText[3].text += ('0'+(int)((float)N*0.1));
	mText[3].text += ('0'+N-(int)((float)N*0.1)*10);
	mText[3].numChar = 6;
	}
	else if(N<10)
	{
...
void test1d3d::update(float dt)
{
	...
	mLightPos.x = 12.0*sin(mtime);
	mLightPos.z = 12.0*cos(mtime);
	mLightPos.y = 6.0f;
	lightPos = mLightPos;
	mLightVec = mLightPos;
	D3DXVec3Normalize(&mLightVec, &mLightVec);
	mCamPos.y = 7.0f;// * sin(mtime*0.2f);
	mCamPos.x = 20.0f * cos(0.2f*mtime);
	mCamPos.z = 20.0f * sin(0.2f*mtime);
	/*
	vy += -10.0f*dt;
	y += vy*dt;
	if(y <= 0.5f)
	...
	*/
	//fout << "y_index=" << y_index  << std::endl;
	//fout << "dt=" << dt  << std::endl;
	
	for(int i = 1; i < MESHN-1; ++i)
	{
		for(int j = 1; j < MESHM-1; ++j)
		{
		
		
		
		
		height1[y_index][i][j] = calcHeight(i,j,dt,mtime,y_index);
		//fout << "(i,j)=(" << i << "," << j << "), height = " << height1[y_index][i][j]<< std::endl;
		
		}
	}
	if(height1[y_index][25][25] < 6.0f*sin(mtime) )
		height1[y_index][25][25] = 6.0f*sin(mtime);
	if(height1[y_index][75][75] < -8.0f*cos(0.7f*mtime) )
		height1[y_index][75][75] = -8.0f*cos(0.7f*mtime);
	if(height1[y_index][55][45] < -8.0f*cos(0.2f*mtime) )
		height1[y_index][55][45] = -8.0f*cos(0.2f*mtime);
	if(height1[y_index][48][52] > 10.0f*cos(0.6f*mtime) )
		height1[y_index][48][52] = 10.0f*cos(0.6f*mtime);
	//if(height1[y_index][50+(int)(20.0f*cos(mtime))][50+(int)(20.0f*sin(0.5f*mtime))] < 2.0f )
	//	height1[y_index][50+(int)(20.0f*cos(mtime))][50+(int)(20.0f*sin(0.5f*mtime))] = 2.0f;
	
	//fout << y_index << std::endl;
	setViewMtx();
...
void test1d3d::draw()
{
	...
	if(FAILED(mVBLTGrid->Unlock()))
		...
		*/
		...
		drawLTArrow(D3DXVECTOR3((25.0f-50.0f)*0.25f, 6.0f*sin(mtime)-6.0f,(25.0f-50.0f)*0.25f),6.0f,R, mRedTex);
		drawLTArrow(D3DXVECTOR3((75.0f-50.0f)*0.25f, -8.0f*cos(0.7f*mtime)-6.0f,(75.0f-50.0f)*0.25f),6.0f,R, mBlueTex);
		drawLTArrow(D3DXVECTOR3((45.0f-50.0f)*0.25f, -8.0f*cos(0.2f*mtime)-6.0f,(55.0f-50.0f)*0.25f),6.0f,R, mBlueTex);
		D3DXMatrixRotationX( &R, D3DX_PI);
		drawLTArrow(D3DXVECTOR3((52.0f-50.0f)*0.25f, 10.0f*cos(0.6f*mtime)+6.0f,(48.0f-50.0f)*0.25f),6.0f,R, mGreenTex);
		//d3dDrawLine(&pointLine[0],5, &LineVec, &lineW);
		d3dDrawLine(NULL,5, &LineVec, &lineW);
	...
	for(int i = 0; i < MESHN; ++i)
	{
		...
		DWORD k = i*MESHN + j;
			...
			vpnt[k].pos.y = height1[y_index][i][j];
	...
	if(y_index>2)
		...
		drawLTXYZaxis(D3DXVECTOR3(0.0f,0.0f,0.0f),16.0f);
	...
	md3dDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA);
		...
		drawLTSpring();
		*/
		mSpherePos = mLightPos;
		mSphereScale = D3DXVECTOR3(0.4f, 0.4f, 0.4f);
		drawSphere();
		//mSpherePos = D3DXVECTOR3(0.0f, 3.0f, 0.0f);
		//mSphereScale = D3DXVECTOR3(1.0f, 1.0f, 1.0f);
		//drawSphere();
		/*
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpsCmdLine, int nCmdShow)
{
	...
	myd3d->mParticle = new test1particle(myd3d, "particle.fx", "particleTech", "..\\resource\\img\\torch.dds", box, 2500, 0.001f);
...
float calcHeight(int i, int j, float dt, float time, int yind)
{
	...
	{
	case 0:
		height = 2.0f*height1[2][i][j] - height1[1][i][j] + (TX*(height1[2][i+1][j]-2.0f*height1[2][i][j]+height1[2][i-1][j]) + TY*(height1[2][i][j+1]-2.0f*height1[2][i][j]+height1[2][i][j-1]))*dt*dt*0.2f + (fPerMass(i,j,time)*dt - DC*(height1[2][i][j]-height1[1][i][j]))*dt;
		//fout << "y_index=" << y_index << "(i,j)=(" << i << "," << j << "), height = " << height<< std::endl;
		break;
	case 1:
		height = 2.0f*height1[0][i][j] - height1[2][i][j] + (TX*(height1[0][i+1][j]-2.0f*height1[0][i][j]+height1[0][i-1][j]) + TY*(height1[0][i][j+1]-2.0f*height1[0][i][j]+height1[0][i][j-1]))*dt*dt*0.2f + (fPerMass(i,j,time)*dt - DC*(height1[0][i][j]-height1[2][i][j]))*dt;
		//fout << "y_index=" << y_index << "(i,j)=(" << i << "," << j << "), height = " << height<< std::endl;
		break;
	case 2:
		//fout << height1[1][i][j] << "," << height1[0][i][j] << std::endl;
		height = 2.0f*height1[1][i][j] - height1[0][i][j] + (TX*(height1[1][i+1][j]-2.0f*height1[1][i][j]+height1[1][i-1][j]) + TY*(height1[1][i][j+1]-2.0f*height1[1][i][j]+height1[1][i][j-1]))*dt*dt*0.2f + (fPerMass(i,j,time)*dt - DC*(height1[1][i][j]-height1[0][i][j]))*dt;
		//fout << "y_index=" << y_index << "(i,j)=(" << i << "," << j << "), height = " << height<< std::endl;
		break;
	default:
		height = 0.0f;
...
void onChange(IDirect3DVertexBuffer9* buf,std::vector<MYTEXT3D> mText)
{
	/*
	VPosNormalTex* vpnt = 0;
	if(FAILED(buf->Lock(0, 0, (void**)&vpnt, 0)))
		MessageBox(NULL, "VertexBuffer Lock() failed", "", MB_OK);
	...
	for(int i = 0; i <= 100; ++i)
	{
		...
		vpnt[k].pos.y = 10.0f*calcHeight(-5.0f+(float)i/10.0f, -5.0f+(float)j/10.0f, N, M);
		if(i != 0 && j != 0)
		{
...
float initialPos(int i,int j)
{
	...
	*/
	return (float)(i*(MESHN-1-i)*j*(MESHM-1-j))*0.000001f;
}
